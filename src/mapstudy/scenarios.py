...
def noisy_detector(gt: GroundTruth, image: ImageRecord, rng: np.random.Generator) -> list[Detection]:
    """Baseline: random jitter of position and size, with an uninformative score.

    Each coordinate is perturbed by up to ``BASELINE_JITTER`` of the box size and the
    result is clipped to the image. Scores are drawn uniformly, independently of IoU.
    """
    x, y, w, h = gt.box
    dx, dy, dw, dh = rng.uniform(-BASELINE_JITTER, BASELINE_JITTER, size=4) * (w, h, w, h)
...
def sub_threshold_detector(gt: GroundTruth, image: ImageRecord, rng: np.random.Generator) -> list[Detection]:
    """Scenario A: confident predictions that sit just below the IoU = 0.5 threshold.

    Every box keeps its size and is shifted by ``SCENARIO_A_SHIFT`` of its width and
    height towards the bottom-right, giving ``IoU = diagonal_shift_iou(0.2) ≈ 0.471``
    for every object. The shift that would give exactly 0.5 is ``1 - sqrt(2/3) ≈ 0.184``.
    """
    x, y, w, h = gt.box
    box = (x + SCENARIO_A_SHIFT * w, y + SCENARIO_A_SHIFT * h, w, h)
    score = float(rng.uniform(*SCENARIO_A_SCORE_RANGE))
    return [Detection(gt.image_id, gt.category_id, box, score)]


def hedging_detector(gt: GroundTruth, image: ImageRecord, rng: np.random.Generator) -> list[Detection]:
    """Scenario B: one accurate box plus several low-confidence "hedging" boxes.
    ...
    for _ in range(SCENARIO_B_N_HEDGES):
...
def generate_detections(images: Sequence[ImageRecord], scenario: str, seed: int) -> list[Detection]:
    """Apply a scenario to every object of every image, reproducibly for a given seed."""
    detector = SCENARIOS[scenario].detector
