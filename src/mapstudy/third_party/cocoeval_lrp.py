# Third-party code, vendored without functional changes.
#
# This is the COCO API evaluator (``pycocotools.cocoeval``, Piotr Dollar and
# Tsung-Yi Lin, Simplified BSD License) extended with the optimal LRP metric of
# Oksuz et al., "Localization Recall Precision (LRP): A New Performance Metric
# for Object Detection", ECCV 2018. The AP computation is identical to
# pycocotools; ``accumulate`` additionally fills ``eval['olrp*']`` and
# ``summarize`` reports oLRP and its components in ``stats[12:16]``.
#
# It is excluded from linting to keep the diff with upstream minimal.
